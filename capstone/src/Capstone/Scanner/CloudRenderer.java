...
public class CloudRenderer extends AbstractRenderer {
	...
	private int[] quater = new int[] { one, one, -one, -one, one, -one, one,
			one, one, -one, one, one,
	...
	public CloudRenderer(Context context) {
		...
		ByteBuffer vbb = ByteBuffer
				.allocateDirect(PointCloud.vertices.length * 4);
	...
	protected void draw(GL10 gl) {
		...
		// gl.glPushMatrix();
		// gl.glTranslatef(PointCloud.vertices[i], PointCloud.vertices[i+1],
